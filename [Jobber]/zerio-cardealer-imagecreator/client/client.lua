...
Citizen.CreateThread(function()
    ...
    function StartScreenShoting()
        ...
        for i,v in pairs(VehiclesFromDB) do
            ...
            if not screenshot then print("breaking coz no screenshot (???)") break end
            ...
            if IsModelInCdimage(hashmodel) then
                ...
                exports['screenshot-basic']:requestScreenshotUpload('https://api.fivemanage.com/api/image', 'image', {
                    headers = {
                        Authorization = 'vaPPNIKMS2xVH63LaIfzc4C0BYWgqMAI',
                    }
                }, function(data)
                    ...
                    if image and image.url then
                        ...
                        print("failed uploading image", image.url,image.status, image.error, image.message) 
                    end
                end)
                Citizen.Await(wait)
            end
