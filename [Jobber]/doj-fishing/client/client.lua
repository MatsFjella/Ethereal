	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Config.StopFishing) then
				endFishing()
				QBCore.Functions.Notify('Du stoppet å fiske', 'error')
			end
			...
			if fishing then
				...
				if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z -3.0)  then
					...
					endFishing()
					QBCore.Functions.Notify('Vannet er ikke dypt nok for å fiske', 'error')
				end
				if IsEntityDead(playerPed) or IsEntityInWater(playerPed) then
					endFishing()
					QBCore.Functions.Notify('Sluttet å fiske', 'error')
				end
			end
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					QBCore.Functions.Notify('Fisken rømte!', 'error')
...
CreateThread(function()
	while true do
		...
		if fishing then
			pause = true
			correct = 1
			TriggerEvent('3dme:triggerDisplay', 'Noe rykket i fiskestangen!')
			exports["qb-core"]:DrawText("Trykk [F] for å fange fisken")
			input = 0
			pausetimer = 0
		end
	end
...
RegisterNetEvent('fishing:client:attemptTreasureChest', function()
	...
	if HasItem then 
		QBCore.Functions.Progressbar("accepted_key", "Låser opp kisten..", (math.random(2000, 5000)), false, true, {
	  ...
	  QBCore.Functions.Notify("Du har ikke en nøkkel som passer!", "error")
	end
end)
...
RegisterNetEvent('fishing:client:useFishingBox', function(BoxId)
	TriggerServerEvent("inventory:server:OpenInventory", "stash", 'FishingBox_'..BoxId, {maxweight = 250000, slots = 250})
...
RegisterNetEvent('fishing:fishstart', function()
	...
	local pos = GetEntityCoords(playerPed) 
	if IsPedSwimming(playerPed) then return QBCore.Functions.Notify("Du kan ikke svømme og fiske samtidig", "error") end 
	if IsPedInAnyVehicle(playerPed) then return QBCore.Functions.Notify("Gå ut av kjøretøyet for å fiske", "error") end 
	if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z - 3.0)  then
		local time = 1000
		-- QBCore.Functions.Notify('Using Fishing Rod', 'primary', time)
		Wait(time)
		exports["qb-core"]:DrawText("Trykk [X] for å stoppe å fiske") 
		fishAnimation()
	else
		QBCore.Functions.Notify('Du må gå lengre bort fra vannet', 'error')
	end
end, false)
            ...
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                    if IsBoatAnchoredAndFrozen(currVeh) then
						QBCore.Functions.Progressbar("random_task", "Hever ankeret", 5000, false, true, {
						...
						-- 	QBCore.Functions.Notify('Heiser opp ankeret', 'success')
                        -- Wait(2000)
						-- QBCore.Functions.Notify('Ankeret er hevet', 'primary')
                    ...
                    elseif not IsBoatAnchoredAndFrozen(currVeh) and CanAnchorBoatHere(currVeh) and GetEntitySpeed(currVeh) < 3 then
						QBCore.Functions.Progressbar("random_task", "Senker ankeret", 5000, false, true, {
							disableMovement = false,
							disableCarMovement = false,
							disableMouse = false,
							disableCombat = true,
							...
							-- QBCore.Functions.Notify('Løsner ankeret', 'primary')
							-- Wait(2000)
							-- QBCore.Functions.Notify('Ankeret er senket', 'success')
...
loseBaitAnimation = function()
	...
	exports["qb-core"]:DrawText("Fisken spiste agnet ditt")
	Wait(2000)
	exports["qb-core"]:HideText()
	fishAnimation()
end
...
catchAnimation = function()
	...
	TaskPlayAnim(ped, animDict, animName, 1.0, -1.0, 1.0, 0, 0, 0, 48, 0)
	local time = 1750
	QBCore.Functions.Notify('Fisk fanget!', 'success', time)
	Wait(time)
	TriggerServerEvent('fishing:server:catch') 
	loseBait()
...
fishAnimation = function()
	...
	if HasItem then
	  ...
	  endFishing()
	  QBCore.Functions.Notify("Du har ikke agn for å fiske", "error")
	end
end
...
attemptTreasureChest = function()
	...
	RemoveAnimDict(animDict)
	QBCore.Functions.Notify('Forsøker å åpne kisten..', 'primary', 1500)
	Wait(1500)
...
openedTreasureChest = function()
	if math.random(1,15) == 10 then
		...
		QBCore.Functions.Notify("Nøkkelen knakk", "error", 7500)
	end
	...
	QBCore.Functions.Notify("Kisten ble åpnet, husk å ta alle tingene!", "success", 7500)
	local ShopItems = {} 
	ShopItems.label = "Skattekiste"
