...
UI.Print = async function(data) {
    ...
    if ($("#filename").val()) {
        if (isValidHttpUrl($("#fileurl").val())){
            ...
            var w = end.width;
            var h = end.height;
            $.post(`https://${GetParentResourceName()}/PrintDocument`, JSON.stringify({
                    url: $("#fileurl").val(),
                    name: $("#filename").val(),
                    amount: $("#amountpapers").val(),
                    width: w,
                    height: h
            }));
        }
        else{
            $.post(`https://${GetParentResourceName()}/Invalid`);
        }
    }
    else{
        $.post(`https://${GetParentResourceName()}/EmptyName`);
    }
