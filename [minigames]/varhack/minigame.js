...
let validate = (ev) => {
    ...
    if (parseInt(ev.target.dataset.number, 10) === order) {
        ...
        if(order > numbers){
            ...
            document.querySelector('.splash .text').innerHTML = 'Sikkerhets sjekk godkjent';
        ...
        document.querySelector('.splash .text').innerHTML = 'Sikkerhetssjekk avslått';
        document.querySelector('.splash').classList.remove('hidden');
        document.querySelector('.groups').classList.add('hidden');
        //document.querySelector('.groups').classList.remove('playing');
...
let destroy = () => {
    ...
    document.querySelector('.sub_text').innerHTML = '';
    setTimeout(function() {
        document.querySelector('.splash .text').innerHTML = 'Fingeravtrykk ikke gjenkjent';
        document.querySelector('.sub_text').innerHTML = 'Bevis på trening nødvendig';
    },500)
}
...
window.addEventListener('message', (event) => {
    if (event.data.type === 'open') {
        ...
        sleep(3000, function() {
            document.querySelector('.splash .text').innerHTML = 'STARTER...';
            document.querySelector('.splash').classList.add('hidden');
            document.querySelector('.groups').classList.remove('hidden','playing');
            start();
        });
