...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        if Config.Progressbar.Enable then
            QBCore.Functions.Progressbar('open_inventory', 'Åpner inventar...', math.random(Config.Progressbar.minT, Config.Progressbar.maxT), false, true, { -- Name | Label | Time | useWhileDead | canCancel
...
RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    QBCore.Functions.Progressbar("repair_vehicle", "Lager..", (math.random(2000, 5000) * amount), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
        QBCore.Functions.Notify("Feilet", "error")
        isCrafting = false
	end)
end)
...
RegisterNetEvent('inventory:client:PickupSnowballs', function()
    ...
    QBCore.Functions.Progressbar("pickupsnowball", "Samler snøballer..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        QBCore.Functions.Notify("Kansellert", "error")
    end)
end)
                ...
                if vehicle ~= 0 and vehicle ~= nil then
                    ...
                    if #(pos - trunkpos) < 1.5 and not IsPedInAnyVehicle(ped) then
                        if GetVehicleDoorLockStatus(vehicle) < 2 then
                            ...
                            QBCore.Functions.Notify("Kjøretøy låst.", "error")
                            return
                        end
                    else
                        CurrentVehicle = nil
...
RegisterNUICallback("GiveItem", function(data, cb)
    ...
    if player ~= -1 and distance < 3 then
        if data.inventory == 'player' then
            ...
            QBCore.Functions.Notify("Du har ikke gjenstand!", "error")
        end
    else
        QBCore.Functions.Notify("Ingen i nærheten!", "error")
    end
    cb('ok')
end)
