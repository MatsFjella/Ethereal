...
local function AddItem(source, item, amount, slot, info, created)
	...
	if not itemInfo and not Player.Offline then
		QBCore.Functions.Notify(source, "Gjenstand finnes ikke", 'error')
		return false
	end
	...
	elseif not Player.Offline then
		QBCore.Functions.Notify(source, "Inventar fullt", 'error')
	end
	return false
end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount, created)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent("QBCore:Notify", source, "Du har ikke denne gjenstanden!", "error")
		return
	end
end
	...
	elseif itemData.useable then
		...
		TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "use")
					...
					if itemInfo['delete'] and RemoveItem(src,itemData.name,1,slot) then
						...
						TriggerClientEvent("QBCore:Notify", src, "Du kan ikke bruke denne", "error")
					end
				end
			else
				UseItem(itemData.name, src, itemData)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				if IsItemValid then
					...
					TriggerClientEvent('QBCore:Notify', src, "Du kan ikke selge denne..", 'error')
				end
			else
				-- drop
			...
			TriggerClientEvent("QBCore:Notify", src, "Du har ikke denne gjenstanden!", "error")
		end
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			QBCore.Functions.Notify(src, "Gjenstand finnes ikke", "error")
		end
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
        ...
        elseif bankBalance >= price then
                ...
                QBCore.Functions.Notify(src, "Du har ikke nok penger..", "error")
            end
		else
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Du har ikke riktige ting..", "error")
		end
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if Player == OtherPlayer then return QBCore.Functions.Notify(src, "Du kan ikke gi deg selv ting?") end
	if dist > 2 then return QBCore.Functions.Notify(src, "Du er for langt unna!") end
	local item = GetItemBySlot(src, slot)
	if not item then QBCore.Functions.Notify(src, "Gjenstand du prøvde å gi finnes ikke!"); return end
	if item.name ~= name then QBCore.Functions.Notify(src, "Feil gjenstand prøv på nytt!"); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			...
			TriggerClientEvent('QBCore:Notify', src,  "Du har ikke nok av gjenstanden", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Du har ikke nok til å overføre")
	end
end)
...
QBCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('QBCore:Notify', source,  "Ikke en gyldig type..", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "Du har ikke fyllt ut korrekt..", "error")
	end
end, "admin")
	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					QBCore.Functions.Notify(source, "Du har gitt " ..GetPlayerName(id).." "..amount.." "..itemData["name"].. "", "success")
				else
					QBCore.Functions.Notify(source, "Kan ikke gi gjenstand!", "error")
				end
			else
				QBCore.Functions.Notify(source, "Gjenstand finnes ikke", "error")
			end
	else
		QBCore.Functions.Notify(source,  "Person er ikke våken", "error")
	end
end, "admin")
...
QBCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    ...
    if Player then
        ...
        QBCore.Functions.Notify(source, "Person ikke våken", 'error')
    end
end, 'admin')
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Fornavn:</strong> {1} <br><strong>Etternavn:</strong> {2} <br><strong>Fødselsdato:</strong> {3} <br><strong>Lisens:</strong> {4}</div></div>',
					args = {
						"Førerkort",
						item.info.firstname,
						item.info.lastname,
...
CreateUsableItem("id_card", function(source, item)
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Personnummer:</strong> {1} <br><strong>Fornavn:</strong> {2} <br><strong>Etternavn:</strong> {3} <br><strong>Fødselsdato:</strong> {4} <br><strong>Kjønn:</strong> {5} <br><strong>Nasjonalitet:</strong> {6}</div></div>',
					args = {
						"ID kort",
						item.info.citizenid,
						item.info.firstname,
