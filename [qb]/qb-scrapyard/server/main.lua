...
RegisterNetEvent('qb-scrapyard:server:ScrapVehicle', function(listKey)
    ...
    if Config.CurrentVehicles[listKey] ~= nil then
        for _ = 1, math.random(2, 8), 1 do
            local item = Config.Items[math.random(1, #Config.Items)]
            Player.Functions.AddItem(item, math.random(35, 55))
        ...
        local moneyAmount = math.random(1000, 5000) -- Random amount between $1,000 and $5,000
        Player.Functions.AddMoney('bank', moneyAmount) -- Assuming money is added to the player's bank account. Adjust accordingly.
        TriggerClientEvent('QBCore:Notify', src, "You've received $" .. moneyAmount .. " for scrapping the vehicle.", 'success')
