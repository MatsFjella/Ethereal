...
statusChecks = {}
statusCheckTime = 0
healAnimDict = 'anim@mp_player_intcelebrationfemale@knuckle_crunch'
healAnim = 'knuckle_crunch'
injured = {}
...
RegisterNetEvent('hospital:client:SetBedPlayer', function()
    Player = QBCore.Functions.GetPlayerData()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if GetAvailableBed(closestBed) then
            TriggerServerEvent("hospital:server:SendToBedPlayer", playerId, closestBed, false)
        else
