...
AddEventHandler('qb-communityservice:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and not communityServiceFinished do
		...
		if mesafe > 45 then
			...
			QBCore.Functions.Notify("Du kan ikke unnslippe. Samfunnstjenesten din er utvidet.")
		end
		Citizen.Wait(20000)
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if actionsRemaining > 0 and isSentenced then
			time = 1
			draw2dText("~b~" .."You have " ..QBCore.Shared.Round(actionsRemaining).. ' ~b~ ~s~ flere handlinger å fullføre før du kan fullføre tjenesten.', { 0.70, 0.955 } )
			...
			for i = 1, #availableActions do
				...
				if distance < 1.5 then
					QBCore.Functions.DrawText3D(pCoords.x,pCoords.y,pCoords.z, "[E] Tøm")
					...
					if(IsControlJustReleased(1, 38))then
						...
						if (tmp_action.type == "cleaning") then
							...
							exports["rpemotes"]:EmoteCommandStart("broom", textureVariation)
							-- TaskStartAnim(PlayerPedId(), "broom", 0, false)
						...
						if actionsRemaining == 0 then
							QBCore.Functions.Notify("Du har fullført samfunnstjenesten", "success")
