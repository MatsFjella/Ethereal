...
local kamu = data.kamu
	...
	if xPlayer.PlayerData.job.name == "police" then
		...
		if tPlayer then
			...
			tPlayer.Functions.SetMetaData("communityservice", count)
			TriggerClientEvent("QBCore:Notify", src, "Spilleren er dømt til samfunnsstraff.")
		else
			TriggerClientEvent("QBCore:Notify", source, "Det finnes ingen slik spiller!", "error")
				end
	end
end)

QBCore.Commands.Add("avsluttsamfunnsstraff", "", {{name="id", help="Player ID"}}, true, function(source, args) -- name, help, arguments, argsrequired,  end sonuna persmission
