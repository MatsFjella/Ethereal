...
RegisterNetEvent('inventory:server:RobPlayer', function(targetPlayerId)
    local src = source
    ...
    if target then
        ...
        if targetCash > 0 then
            -- Remove cash from the target player
            target.Functions.RemoveMoney('cash', targetCash, "Ranet av en annen spiller")
            -- Add cash to the robber
            robber.Functions.AddMoney('cash', targetCash, "Ranet spiller")
            TriggerClientEvent('QBCore:Notify', src, 'Du har ranet kr' .. targetCash, 'success')
            TriggerClientEvent('QBCore:Notify', targetPlayerId, 'Du har blitt ranet for kr' .. targetCash, 'error')
        else
            TriggerClientEvent('QBCore:Notify', src, 'Person har ikke penger', 'error')
        end
    end
end)
