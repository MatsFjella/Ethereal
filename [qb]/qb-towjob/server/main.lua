...
RegisterNetEvent('qb-tow:server:nano', function(vehNetID)
    ...
    if chance < 26 then
        Player.Functions.AddItem('markertesedler', 600, false)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markertesedler'], 'add')
    end
end)
...
RegisterNetEvent('qb-tow:server:11101110', function(drops)
    ...
    local bonus = 0
    local DropPrice = math.random(1500, 3500)
    if drops > 5 then
