...
exports('ubatran', FleecaBankRobbery)

local function FleecaBankRobbery(camId)
    local coords = GetEntityCoords(cache.ped)

    local dispatchData = {
        message = locale('ubatran'),
        codeName = 'ubat',
        code = '10-90',
        icon = 'fas fa-vault',
        priority = 2,
...
local function PhoneCall(message, anonymous, job, type)
    local coords = GetEntityCoords(cache.ped)
    local playerServerId = GetPlayerServerId(PlayerId()) -- For local client-side scripts
    ...
    if IsCallAllowed(message) then
        ...
        local dispatchData = {
            ...
            alertTime = nil,
            jobs = job,
            playerID = playerServerId  -- Adding player's server ID
        }
