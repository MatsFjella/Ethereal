...
function Koci.Client.HUD:SeatBeltLogic(vehicle)
    if Config.Settings.Seatbelt.active then
        if self.data.vehicle.inVehicle and not self.data.vehicle.isSeatbeltOn then
            ...
            if (isVehMovingFwd and speedIsBiggerThan and reallyFast) then
                self:EjectSeat()
            else
