...
RegisterNetEvent('ps-fuel:client:ShowCanInput', function ()
	...
	local dialog = exports['qb-input']:ShowInput({
		header = "Bensinstasjon",
		submitText = "Beløp: kr"..Config.canCost,
		inputs = {
			{
				text = "Betalningsmetode",
				name = "billtype",
				type = "select",
				options = {
					{ value = "cash", text = "Kontakt" },
					{ value = "bank", text = "Kort" }
				},
			},
		},
	})
...
RegisterNetEvent('ps-fuel:client:refuelCanMenu', function()
	...
	if fuelToAdd ~= 0 then
		...
		local dialog = exports['qb-input']:ShowInput({
			header = "Bensinstasjon",
			submitText = "Beløp: kr"..Config.refuelCost,
			inputs = {
				{
					text = "Betalningsløsning",
					name = "billtype",
					type = "select",
					options = { 
						{ value = "cash", text = "Kontanter" },
						{ value = "bank", text = "Kort" }
					},
				},
			},
...
RegisterNetEvent('ps-fuel:client:ShowInput', function (refillCost)
	...
	if not HasPedGotWeapon(ped, 883325847) then
		local dialog = exports['qb-input']:ShowInput({
			header = "Bensinstasjon",
			submitText = "Beløp: kr"..refillCost,
			inputs = {
				{
					text = "Betalningsløsning",
					name = "billtype",
