...
QBCore.Commands.Add('uak', Lang:t("command.ooc.help"), {}, false, function(source, args)
    ...
    for _, v in pairs(Players) do
        TriggerClientEvent('chat:addMessage', v, {
            color = { 0, 0, 255},
            multiline = true,
            args = {'UAK | '.. GetPlayerName(source), message}
        })
    end

    -- Log the message for admin
    TriggerEvent('qb-log:server:CreateLog', 'ooc', 'UAK', 'white', '**' .. GetPlayerName(source) .. '** (CitizenID: ' .. Player.PlayerData.citizenid .. ' | ID: ' .. source .. ') **Message:** ' .. message, false)
end, 'user')
...
--                     color = { 0, 0, 255},
--                     multiline = true,
--                     args = {'Lokal UAK | '.. GetPlayerName(source), message}
--                 })
--                 TriggerEvent('qb-log:server:CreateLog', 'ooc', 'UAK', 'white', '**' .. GetPlayerName(source) .. '** (CitizenID: ' .. Player.PlayerData.citizenid .. ' | ID: ' .. source .. ') **Message:** ' .. message, false)
--             end
--         end
...
QBCore.Commands.Add('meg', Lang:t("command.me.help"), {{name = Lang:t("command.me.params.message.name"), help = Lang:t("command.me.params.message.help")}}, false, function(source, args)
...
QBCore.Commands.Add('jc', 'Politi', {{name='Melding', help='Melding å sende'}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
  
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        if args[1] ~= nil then
            local msg = table.concat(args, ' ')
            local firstName = Player.PlayerData.charinfo.firstname  -- Get the player's first name
            local players = QBCore.Functions.GetQBPlayers()
            for _, Player in pairs(players) do
              if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
                -- Include the player's first name in the message
                TriggerClientEvent('chatMessage', Player.PlayerData.source, 'Politi ', 'info', firstName .. ": " .. msg)
                TriggerClientEvent('police:PlaySound', Player.PlayerData.source, 'Event_Start_Text', 'GTAO_FM_Events_Soundset', 0, 0, 1)
              end
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Du må fylle ut melding!', 'error', 5000)
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Denne kommandoen er for politiet!', 'error', 5000)
    end
end)

QBCore.Commands.Add('ambulanse', 'ambulanse', {{name='Melding', help='Melding å sende'}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
  
    if Player.PlayerData.job.type == 'ems' and Player.PlayerData.job.onduty then
        if args[1] ~= nil then
            local msg = table.concat(args, ' ')
            local firstName = Player.PlayerData.charinfo.firstname  -- Get the player's first name
            local players = QBCore.Functions.GetQBPlayers()
            for _, Player in pairs(players) do
              if Player.PlayerData.job.type == 'ems' and Player.PlayerData.job.onduty then
                -- Include the player's first name in the message
                TriggerClientEvent('chatMessage', Player.PlayerData.source, 'Ambulanse ', 'info', firstName .. ": " .. msg)
                TriggerClientEvent('police:PlaySound', Player.PlayerData.source, 'Event_Start_Text', 'GTAO_FM_Events_Soundset', 0, 0, 1) -- Sound effect for messages
              end
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Du må fylle ut melding!', 'error', 5000)
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Denne kommandoen er for ambulanse!', 'error', 5000)
    end
end)
