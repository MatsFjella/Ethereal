...
<section class="transactions-container">
  <input
    ...
    placeholder="Søk i transaksjoner (Melding, ID, Mottaker)..."
  ...
  <section class="scroller">
    {#if account}
      {#if account.transactions.filter((item) => item.message
        ...
        {/each}
      {:else}
        <h3 style="text-align: left; color: #F3F4F5; margin-top: 1rem;">Ingen transaksjoner funnet</h3>
      {/if}
    {:else}
  ...
  {#if !isAtm}
    <div class="export-data">
      ...
      >
        ...
        {$translations.export_data || 'Eksporter transaksjonsdata'}</button
      >
    </div>
  {/if}
</section>
